...
@router.put('/{user_id}')
def update_user(user_id: int, user: schemas.UserUpdate, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
    return controllers.update_user(user_id, user, db)

@router.post('/change_password')
def change_password(user: schemas.UserChangePassword, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
...
# @router.post('/wallet/charge/{user_id}')
# def charge_wallet(user_id: int, charge: schemas.WalletCharge, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
#     return controllers.charge_wallet(user_id, charge, db)

# @router.post('/wallet/withdraw/{user_id}')
# def withdraw_wallet(user_id: int, withdraw: schemas.WalletCharge, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
#     return controllers.withdraw_wallet(user_id, withdraw, db)
...
def create_transaction(transaction: schemas.TransactionCreate, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
    return controllers.create_transaction(transaction, db)

@transaction_router.put('/{transaction_id}')
def update_transaction(transaction_id: int, transaction: schemas.TransactionUpdateById, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
    return controllers.update_transaction(transaction_id, transaction, db)

@transaction_router.put('/user/{user_id}')
def update_transaction_by_user_id(user_id: int, transaction: schemas.TransactionUpdateByUserId, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
    return controllers.update_transaction_by_user_id(user_id, transaction, db)

@transaction_router.delete('/{transaction_id}')
def delete_transaction(transaction_id: int, db: Session = Depends(get_db), token: str = Depends(get_current_user)):
